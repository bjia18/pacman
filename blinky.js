var blinky = function() {
	...
	this.update = function(dt) {
		...
		var minDistance(move(newPosition, nocollision[0], length), {x: pacman.x, y: pacman.y});
		...
		for (var i = 1; i < nocollision.length; i++) {
			...
			var currentDistance = distance(move({x: this.x, y: this.y}, nocollision[i], this.step * dt), {x: pacman.x, y: pacman.y});
